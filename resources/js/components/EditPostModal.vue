...
import { Type, Layers, ImageIcon } from 'lucide-vue-next';
...
const schema = yup.object({
    id: yup.number().required(), // نحتاج للـ ID للتحديث
    ...
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 bg-opacity-50">
        <div class="bg-white w-full max-w-lg rounded-xl shadow-lg p-6 relative">
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                <button @click="$emit('close')" class="text-gray-400 hover:text-gray-600 text-3xl" style="cursor: pointer">×</button>
            </div>
            ...
            <form @submit.prevent="onSubmit" class="space-y-5">
                ...
                <!-- Image -->
                <div>
                    <label class="block mb-1 text-sm font-medium text-gray-700">Change Image (optional)</label>
                    <div class="relative">
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit"
                            class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium">Update Post
                    </button>
                </div>
            </form>
        </div>
